...
def get_last_sender(chat_log: str) -> str:
    """
    Returns the sender of the last message in the chat log.
    Works for any name, not just one person.
    """
    # Take the last message (after the last '] ')
    last_message = chat_log.strip().split("] ")[-1]

    # Sender name is before the first ':'
    if ":" in last_message:
        sender = last_message.split(":", 1)[0].strip()
        return sender
    return None
...
while True:
    ...
    # Create a list of all your possible names in the chat
    my_names = ["Tahir", "Tahir Imran", "Tahir Hussain"]

    # Add this new check: only proceed if the last sender is not in the list of my names
    last_sender = get_last_sender(chat_history)

    if last_sender is not None and last_sender not in my_names:
        # The rest of your code for generating and sending a response goes here
